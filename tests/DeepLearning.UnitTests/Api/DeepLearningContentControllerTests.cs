...
namespace DeepLearning.UnitTests.Api
{
    ...
    public class DeepLearningContentControllerTests
    {
        ...
        /// <summary>
        /// <paramref name="userId"/> must be the same identity <paramref name="client"/> is
        /// authenticated as (see CreateAuthenticatedLlmClient) — the question's CreatedBy and any
        /// later submission are always the caller's own JWT identity now (Phase 1 closed the
        /// caller-supplied-UserId trust hole), so a mismatched userId here would make the
        /// UserPatternReview/UserVocabReview assertions below look up the wrong row.
        /// </summary>
        private async Task<(Guid ExamTypeId, Guid QuestionId, Guid UserId)> SeedExamTypeQuestionAndUserAsync(HttpClient client, Guid userId)
        {
        ...
        /// <summary>
        /// Swaps in a fake LLM client and authenticates the returned HttpClient as a freshly
        ...
        // existingUserId lets a caller stand up a second HttpClient (backed by a different
        // ILlmClientResolver substitution — each WithWebHostBuilder call is its own TestServer)
        // that still authenticates as the SAME identity as an earlier client, which matters now
        // that deep-learning content generation/reads enforce question ownership (U4).
        private async Task<(HttpClient Client, Guid UserId)> CreateAuthenticatedLlmClient(
            Action<IServiceCollection> configureLlmClient, Guid? existingUserId = null)
        {
            var userId = existingUserId ?? await _factory.SeedUserAsync(UserRole.admin);
            var client = _factory
