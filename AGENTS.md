...
| `Infrastructure/Ai/{ClaudeLlmClient,OpenAiCompatibleLlmClient,LlmResiliencePipeline,ExamConfigLoader,PromptRenderer}.cs` + `Options/{ClaudeApiOptions,OpenAiCompatibleOptions}.cs` + `Infrastructure/Persistence/Repositories/AiCallLogRepository.cs` | Real — **4 providers wired up**: Claude (own adapter, own request/response shape) plus OpenAI/DeepSeek/Mimo sharing one `OpenAiCompatibleLlmClient` (all three are OpenAI-Chat-Completions-shaped — confirmed against each provider's own docs, not assumed). `ClaudeLlmClient.cs` is a **rename** of the old empty `LlmClient.cs` stub; `LlmResiliencePipeline.cs` is a rename of `ClaudeResiliencePipeline.cs` once it started being shared by all 4. |
...
- **Config**: `Llm:Provider` plus one section per provider in `appsettings.Development.json` (gitignored — real keys live only there, never commit them): `Llm:Claude:{ApiKey,BaseUrl,Model,ApiVersion,WorkspaceId}`, `Llm:{OpenAi,DeepSeek,Mimo}:{ApiKey,BaseUrl,Model,AuthHeaderName,AuthHeaderValuePrefix,MaxTokensFieldName}`. `WorkspaceId` is required for a Claude "identity-linked" API key (hit for real on 2026-08-29: Claude rejected the request with `anthropic-workspace-id is required...`) — grab it from the Console by picking one specific workspace from the workspace dropdown (not "All workspaces", which shows no ID at all).
