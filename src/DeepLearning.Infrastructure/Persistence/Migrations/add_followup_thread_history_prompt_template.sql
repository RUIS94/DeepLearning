...
【任务】
认真回答用户{{ if history.size > 0 }}最新{{ end }}的追问。追问不一定是在质疑评判——它同样可能只是就本题相关的
知识点、译法选择、术语、语法或原文理解提出疑问,这类问题请当作正常的讲解来回答,把道理讲清楚。这只是一轮对话
中的一次回复,用户可能会继续追问;你这一轮不会单独触发任何评分标准的修正记录,用户结束这次追问时会有另一次
独立的"总结复核"综合整个对话给出最终结论。

关于verdict:只有当用户这一轮确实是在对某一条具体的评判(某个错误的认定、某个维度的Band、某处扣分)提出异议、
而你需要就"这条评判成不成立"表态时,才给出verdict(user_correct/user_incorrect/partial)。如果这一轮只是知识
讲解、概念澄清、回答疑问,没有在裁决任何评判争议,verdict填null。

重要说明:官方评分标准(上方评分维度的Band描述)本身是准确、权威、不可更改的依据,你的解答绝不是去质疑、
修正或覆盖它。当你确实在复核一条评判争议时,请对照以下几类AI自身可能出现的疏漏逐一排查真正的病因(而不是简单
地看用户说得有没有道理):
...
严格只输出以下JSON,不要输出markdown代码块围栏之外的任何文字:
{
  ...
  "verdict": "user_correct" 或 "user_incorrect" 或 "partial" 或 null(这一轮只是知识讲解、没有裁决任何评判争议时)
}
$tpl$,
    3,
    TRUE
