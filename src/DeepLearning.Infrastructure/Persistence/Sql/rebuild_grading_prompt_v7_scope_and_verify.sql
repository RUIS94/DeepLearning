-- =====================================================================
-- Grading prompt v7 — take out the coordination the stages do not need, make the
-- checklist teach a method rather than a word list, and let the verdict stage check
-- a comprehension claim instead of trusting a label.
--
-- WHY (2026-09-04, second review pass before freezing):
--
--  1. THE STAGES WERE TOLD ABOUT EACH OTHER FOR NO REASON. The sweep stage opened
--     with "another marker is comparing this translation independently, you cannot
--     see their conclusions" and then had to spend two more lines saying duplicates
--     are fine. None of that is knowledge the stage needs — it has one job and no
--     way to act on the existence of colleagues. It was also self-inflicted: calling
--     the role 复筛员 (re-screener) implies a first screen, which invites exactly the
--     "surely someone already caught this" self-censoring the extra lines then had to
--     argue against. Renamed to 专项排查员 and cut to a single line that says what
--     was actually load-bearing: record everything you find, deciding what is worth
--     recording is not your job.
--
--  2. THE CHECKLIST READ LIKE A LOOKUP TABLE. Headed "逐条对照原文排查;命中即记" with
--     no statement that it is partial, it invites a model to check the listed items,
--     find none, and move on — the same micro-over-macro pull this pipeline was split
--     apart to fix, one level down. The detail itself stays: the evidence stage
--     already works from the general method and no checklist, and it is precisely
--     what missed the subject/object reversal, so stripping the specifics would make
--     the sweep a duplicate of a pass that is known to be insufficient. What changes
--     is the framing — explicitly non-exhaustive, each item a way of looking rather
--     than a string to match, and the per-sentence question is "is there anything of
--     this KIND here", not "does a listed word appear here".
--
--  3. THE VERDICT STAGE COULD NOT CHECK ITS OWN INPUT. Step 2a turns on whether any
--     finding impacts the core message, and it was deciding that from the severity
--     label alone — a value derived upstream from q1/q2/q3, which it had no way to
--     verify. Every q3 that survives now carries the reader's mistaken reading (the
--     handler demotes any that does not), so that line is rendered under each finding
--     and 2a is told to read it rather than the label. Costs a line per finding and
--     converts a trusted number into a checkable claim.
--
-- Explicitly NOT changed: splitting the four stages into four prompt_templates rows.
-- template_type is an ai_operation_type_enum and ExamConfigLoader concatenates every
-- active row for a (exam type, type) pair, so four rows would need new enum values
-- plus a loader change — and would trade one versioned artifact for a four-tuple with
-- no guarantee the four were ever active together. For a pipeline whose whole selling
-- point is reproducibility, "which prompt produced this grading" should stay one id.
--
-- v6 is deactivated, not deleted (AGENTS.md #9).
-- =====================================================================
...
2. 先做两项【事实判定】(此时只判事实,不选 Band):
   a. 是否存在至少一条 major 或 critical?——对应该维度官方描述里 "One or more ... impact the core message / impact the understanding of the target text" 这一支。若该维度的描述没有这一支(即以 accomplished / mostly / some 这类程度措辞区分档位的维度),跳过本项。
      判这一项时,读每条下面那行「读者会误以为」——那是它被算作 impact 的具体依据。不要只看 severity 这个标签就下结论:标签是系统按判据推出来的,依据才是你能核对的东西。
...
-- Verify: exactly one ACTIVE grading row (version 7); v1-v6 kept but inactive.
