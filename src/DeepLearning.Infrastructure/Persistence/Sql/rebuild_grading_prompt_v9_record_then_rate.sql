-- =====================================================================
-- Grading prompt v9 — separate "is this worth recording" from "how bad is it".
--
-- WHY (2026-09-04, third real run). v8 fixed the over-severity — no more dropped
-- article coming back as a major error — but recall collapsed with it: findings went
-- from 34 to 14 on comparable texts, and the evidence stage marked 10 of 13 sentences
-- "ok" and reported two findings.
--
-- The traces put the loss squarely in collection, not in merging (raw per-stage counts
-- fell too, so this is not the span-overlap dedup), and squarely in the stage v8
-- touched. The cause is a spillover v8 created: q3's guidance now says, correctly,
-- "most q1 = true findings are q3 = false" — and a model does not cleanly separate
-- "this is not severe" from "this is not worth reporting". Guidance about SEVERITY was
-- read as guidance about RECORDING.
--
-- The per-sentence status field made it worse rather than better. Deciding ok/deviation
-- first and only then elaborating turns that field into a gate: one cheap judgement
-- now stands between a defect and ever being written down, and any "most things are
-- fine" prior gets applied there, once per sentence, before any evidence is committed
-- to paper.
--
-- v9 does two things, both small:
--   * A firewall line after q3 in all three collection stages: the three questions
--     decide how heavy a deviation is, never whether to record it. Recording is
--     governed by the stage's own opening principles. A finding with q1 = q3 = false
--     still gets written down; it just derives to minor.
--   * "ok" is made the expensive claim it should always have been. It now means "I
--     compared this sentence word by word and there is not one thing", the tie-break
--     is to write deviation and record the finding as minor, and the stage is told
--     outright that wall-to-wall ok means the comparison was not done — not that the
--     translation was perfect.
--
-- Nothing about severity is loosened. v8's q1/q3 separation and the sharpened
-- wrong-reading test are untouched: the goal is the same number of findings as v7 with
-- v8's distribution across them.
--
-- v8 is deactivated, not deleted (AGENTS.md #9).
-- =====================================================================
...
NAATI 官方定义原文:
  ...
  q3 = 它 impacts on comprehension of the target text 吗?
       ...
       【三问只决定这一处有多重,不决定记不记】。记不记由本阶段开头的记录原则决定,与三问无关——
       q3 = false、q1 也 = false 的偏差,同样必须原样记进 findings[],它只是会被推导成 minor。
...
-- Verify: exactly one ACTIVE grading row (version 9); v1-v8 kept but inactive.
