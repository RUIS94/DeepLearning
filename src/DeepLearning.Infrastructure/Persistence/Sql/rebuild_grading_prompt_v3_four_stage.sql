-- =====================================================================
-- Grading prompt v3 — four internally-gated stages in one template row.
--
-- WHY (2026-09-04, from a real graded submission whose bands came out one to two
-- levels too generous — traces logs/ai-trace/0001..0003_20260904-1225*.md):
-- v3 keeps v2's core idea (the official Band text is the only scoring standard, and
-- it lives in a call that sees nothing else) and fixes five things that let a
-- translation with two core-message distortions and eight Chinese-language errors
-- come out as Band 2 / 1 / 1.
--
--  1. SEVERITY IS NO LONGER A LABEL THE MODEL PICKS. v2 asked three yes/no questions
--     and then asked the model to name a level. The audit stage read its own answer
--     "Q1 yes, Q2 no, Q3 no" and wrote `minor` — the correct answer was `moderate` —
--     for four findings in a row, because v2 called the official tier "官方 Minor"
--     and one of the four output values "minor". The model collapsed the two names.
--     Stages now emit the raw q1/q2/q3 booleans plus scopeBeyondSentence, and
--     GradeSubmissionCommandHandler.DeriveSeverity computes the level. A naming
--     collision cannot survive a pure function (AGENTS.md #1).
--
--  2. Q3 NOW USES THE OFFICIAL COMPREHENSION TEST. v2 asked whether the reader would
--     get an understanding "与原文相悖、或读不通" — a far higher bar than NAATI's
--     "impacts on comprehension of the target text". Everything a reader could still
--     roughly follow slid into Minor. Both core-mechanism reversals in the observed
--     submission were scored minor under that wording.
--
--  3. THE SECOND STAGE NO LONGER REVIEWS A LIST — IT COLLECTS INDEPENDENTLY. v2's
--     audit stage was shown stage 1's findings and asked to fix them. It added
--     nothing (`added: []`) while its own reasoning re-read every sentence and said
--     "可以接受" fifteen times, and it downgraded four findings. Being handed a list
--     turns the model into a validator. Stages 2 and 3 now never see earlier
--     findings; the handler takes the UNION and keeps the highest severity per
--     duplicate. Recall is what these passes are for, not consensus.
--
--  4. NEW MONOLINGUAL PROOFREAD STAGE. Reading the translation with the source
--     alongside makes Chinese-side errors invisible — you know what it meant to say.
--     v2 found two language errors in a text that had at least eight (表明着 /
--     带有…疾病 / 体会到灼烧感 / a missing full stop / a 20-character pre-modifier).
--     Stage 2 gets the translation ALONE, with no source and no English, and works
--     as a Chinese copy-editor. Internal terminology inconsistency (晒斑 vs 晒伤,
--     炎症 vs 发炎) is visible monolingually too, which is what finally feeds
--     textual_norms.
--
--  5. textual_norms IS NO LONGER STARVED. v2's routing rule was "first match wins"
--     with "changed the meaning" first, so terminology errors always landed on
--     meaning_transfer, textual_norms received zero evidence, and the verdict stage
--     read zero evidence as Band 1 ("没有证据显示文本规范方面的问题"). Now: the
--     proofread stage owns terminology consistency and non-standard renderings, a
--     term error that also points at a different concept is recorded twice (once per
--     dimension, they are two distinct defects), and the verdict stage is told
--     explicitly that absent evidence is not evidence of absence.
--
--  Also new: the evidence stage must enumerate EVERY source sentence and mark it
--  ok/deviation, so "I stopped after seven findings" is structurally visible. The
--  handler validates the numbering is contiguous from 1.
--
--  Not a prompt problem, fixed separately and worth recording here because it is why
--  the observed grading had no objective anchor at all: PromptRenderer handed Scriban
--  the lazy iterator LINQ's .Select returns, and Scriban's `.size` reports 0 for one.
--  Every `{{ if xxx.size > 0 }}` block in every template silently rendered empty —
--  meaning checkpoints, weak points and correction patches had NEVER reached a
--  grading prompt. The model, still asked for checkpoint verdicts by the output
--  contract, invented thirteen of them. Fixed in PromptRenderer.MaterialiseEnumerables.
--
-- STAGES (the handler passes {{ stage }}; all four render from THIS row):
--   evidence  — bilingual, sentence by sentence, every sentence accounted for.
--               Emits q1/q2/q3 per finding. Sees no Band text.
--   proofread — the TRANSLATION ONLY. No source, no English. Chinese copy-editing +
--               internal terminology consistency. Sees no Band text, no earlier findings.
--   sweep     — bilingual, the easy-to-miss checklist + this learner's weak points +
--               the correction patches. Independent collection, NOT a review: it is
--               never shown what the earlier stages found. Sees no Band text.
--   verdict   — official five-Band descriptions ONLY, over the merged evidence.
--
-- v2 is deactivated, not deleted (AGENTS.md #9).
-- On a fresh DB this must run after rebuild_grading_prompt_three_stage.sql and
-- fix_grading_prompt_v2_verdict_scope_and_drop_reason.sql.
-- =====================================================================
...
NAATI 官方定义原文:
  ...
  q3 = 它 impacts on comprehension of the target text 吗?
       判据按官方口径,门槛不高:只读中文的读者是否需要回读才能理清、或会得到一幅模糊、错位、与原文不符的图景。
       【不要】把门槛抬成"完全读不懂"或"与原文全然相悖"——只要读者被绊住或被引向错误的理解,就是 true。
...
只输出以下 JSON,不要输出任何其他文字,也不要加代码块围栏:
{
  ...
  "findings": [
    {"id": "E1", "positionRef": "<定位,如 第二段第2句>", "sourceTextSnippet": "<原文片段,照抄>", "userTextSnippet": "<译文片段,照抄>", "errorCategory": "<上方 category_key 之一>", "dimensionKey": "<上方维度 key 之一>", "q1": <true|false>, "q2": <true|false>, "q3": <true|false>, "scopeBeyondSentence": <true|false>, "summary": "<≤20字中文定性>", "explanation": "<指出原文哪个成分被改变/遗漏/添加,以及三问的事实依据>", "suggestion": "<改法建议>"}
  ]
}
...
每条同样要回答三问,只答 true/false,不要自己定等级:
  ...
  q2 = 它改变了这段文字的意图或功能吗?
  q3 = 它 impacts on comprehension 吗?——读者是否需要回读才能理清,或会被引向一幅模糊、错位的图景。门槛不高,被绊住即为 true;不要抬成"完全读不懂"。
...
只输出以下 JSON,不要输出任何其他文字,也不要加代码块围栏:
{
  ...
  "findings": [
    {"id": "P1", "positionRef": "<定位,如 第一段第1句>", "sourceTextSnippet": null, "userTextSnippet": "<稿件片段,照抄>", "errorCategory": "<上方 category_key 之一>", "dimensionKey": "<textual_norms|language_proficiency>", "q1": <true|false>, "q2": <true|false>, "q3": <true|false>, "scopeBeyondSentence": <true|false>, "summary": "<≤20字中文定性>", "explanation": "<说明毛病在哪、中文里应当怎么说>", "suggestion": "<改法建议>"}
  ]
}
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  q3 = impacts on comprehension of the target text 吗?——读者需要回读才能理清,或得到模糊、错位、与原文不符的图景即为 true。门槛不高,不要抬成"完全读不懂"。
...
{
  "findings": [
    {"id": "S1", "positionRef": "<定位>", "sourceTextSnippet": "<原文片段,照抄>", "userTextSnippet": "<译文片段,照抄>", "errorCategory": "<上方 category_key 之一>", "dimensionKey": "<上方维度 key 之一>", "q1": <true|false>, "q2": <true|false>, "q3": <true|false>, "scopeBeyondSentence": <true|false>, "summary": "<≤20字中文定性>", "explanation": "<原文依据 + 三问的事实依据>", "suggestion": "<改法建议>"}
  ]
}
...
-- Verify: exactly one ACTIVE grading row (version 3); v1/v2 kept but inactive.
