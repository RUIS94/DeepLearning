-- =====================================================================
-- Grading prompt v2 — CANDIDATE, inserted INACTIVE for A/B testing against v1.
--
-- HOW TO SWAP. ExamConfigLoader CONCATENATES every active row of a template type (it does
-- not pick the highest version), so v1 and v2 must never be active at once:
--     UPDATE prompt_templates SET is_active = (version = 2) WHERE template_type = 'grading';
-- back with (version = 1). Re-running this script is safe; it owns the version-2 row only.
--
-- WHAT CHANGED FROM v1. Numbered so a bad A/B can be bisected.
--
--  1. THE PROMPT ONLY GIVES INSTRUCTIONS NOW. v1 had accumulated a layer of prose that
--     explained itself to a human reader rather than telling the model what to do —
--     rationale for why a stage exists, arguments against the previous version's wording,
--     restatements of what the system does with an answer. All of it removed, no rule
--     dropped. Characters per stage: evidence -15%, proofread -7%, verdict -22%, sweep ±0
--     (sweep is nearly all checklist, which is method rather than commentary and is
--     untouched; proofread's own checklist is likewise most of what remains there).
--
--  2. THE TWO QUESTIONS ARE q1 AND q2. v1 asked q2/q3, a gap left when q1 was removed in v8,
--     which then had to be explained inside the prompt. Renumbering removes the explanation
--     along with the gap. NOTE: the two versions collide — v1's "q2" is v2's "q1" — so
--     Finding binds the raw keys and NormaliseQuestionScheme resolves which pair was
--     answered, keyed on whether q1 or q3 is present. Both prompt versions stay runnable.
--     q3WrongReading is renamed to q2WrongReading to match.
--
--  3. THE MAJOR/MINOR ARROW IS GONE. v1 stated the official definitions AND then spelled out
--     "q2 or q3 true -> Major". Told what the answers add up to, the model answers backwards
--     from the level it wants — the coupling this pipeline removed in the first place. The
--     definitions stay; the mapping does not. The firewall it carried survives as one line:
--     findings answering false to both are still recorded.
--     ^ Riskiest edit in v2. If recall drops, revert this one first.
--
--  4. WHAT TO RECORD IS STATED POSITIVELY. v1 listed exclusions and then argued with itself
--     about their edges. v2 gives a 不记 list and a 必记 list. Register moves into 必记:
--     inappropriate_register is a scoreable NAATI category that a model looking for a reason
--     to skip will otherwise read as a style preference, and the bilingual stages are the
--     only ones that can see a formal source rendered colloquially.
--
--  5. CHECKPOINTS GAIN "contradicted". hit / partial / miss had nowhere for "the translation
--     asserts the opposite", which was being flattened into miss — an omission, a Minor
--     shape — when it is the canonical comprehension failure. BuildCheckpointVerdicts passes
--     the string through unvalidated, so this needs no code change.
--
--  6. THE PROOFREAD STAGE DECLARES NO GENRE AND NO SOURCE TEXT. v1 opened "你是中文科普稿件的
--     责任校对" and measured register against 科普; this bank holds policy statements, public
--     information leaflets and government notices, so the yardstick was wrong for nearly
--     every question. v1 also spent its opening on there being no source text, planting the
--     frame it wanted gone, and asked whether a defect "改变了 intent" — unanswerable with
--     nothing to have changed it from. The prohibitions survive in copy-editing terms; q1 is
--     re-asked as an internal-consistency question; genre is inferred from the manuscript.
--
--  7. THE VERDICT STAGE LOSES ITS HOUSE PROCEDURE. Deleted: the 200-word-vs-2000-word example
--     (this exam's source is fixed at ~250 words) and the instruction that 受影响句子的占比
--     "才是判据" — a criterion this project invented, competing with the official Band text in
--     the one stage where that text should be the only thing in the room. coverage_note stays
--     as plain fact so "多不多" is still not answered from an impression. Band words are to be
--     read at face value rather than converted into a ratio.
--
--  8. AN EMPTY DIMENSION MAY BE RE-READ. v1 froze the evidence absolutely, then asked for a
--     confirmation it had no permission to perform. v2 lifts the freeze for that one case,
--     and only for that dimension. It still emits no findings — those would bypass the merge,
--     the dedup and the severity derivation — and a problem found this way caps confidence at
--     medium.
--
--     The fallback when nothing can be confirmed is now "not Band 1" rather than v1's "Band 2
--     at best". A single number could not mean the same thing in five dimensions: the pass
--     line is Band 2 for meaning_transfer, language_proficiency and revision_skills but Band 3
--     for textual_norms and error_categorisation, so capping at 2 sat exactly on the line for
--     the first three and a full band ABOVE it for the other two — toothless in precisely the
--     dimension that most often comes back with no evidence at all. It also smuggled pass-line
--     reasoning into the one stage told not to consider pass lines (rule 5). Band 1 is an
--     affirmative claim in every dimension (consistently / accomplished / minimal or no /
--     no or almost no / all or almost all), so "cannot confirm, therefore cannot assert it" is
--     the rubric's own logic and needs no number.
--
-- UNCHANGED: Chinese instructions with the official rubric quoted verbatim in English; the
-- four stages; the isolation between the three collection passes; severity derived in code.
-- =====================================================================

BEGIN;

DELETE FROM prompt_templates
WHERE template_type = 'grading' AND version = 2;
...
-- Verify: two grading rows, v1 active and v2 inactive.
