-- =====================================================================
-- Grading prompt v8 — stop q3 from being a restatement of q1.
--
-- WHY (2026-09-04, second real article: a green-IT text). Three findings came back
-- at major that plainly are not:
--   * "The environmentalists" rendered 环境保护论者 rather than 这些环境保护论者;
--   * "international activist group" rendered 激进组织 rather than 行动组织;
--   * "coal-fired electricity" rendered 煤炭发电 rather than 燃煤发电.
-- A Chinese reader gets the right information from all three. Under NAATI these are
-- Minor by definition: they do not impact on the comprehension of the target text.
--
-- The cause was in this file, not in the model. q1 and q3 had been given the SAME
-- list of categories:
--     q1 = ... 指称对象、数量与范围、逻辑关系、时间/体、情态强度
--     q3 = true when ... 指称对象、逻辑关系、范围、施事受事被改写
-- so any q1 = true read as q3 = true — and q3 alone makes an error officially Major.
-- The q3 = false branch made it worse by listing only stylistic faults (clumsy
-- wording, redundancy, word order, register, typos), leaving nothing with a content
-- component anywhere to land except true. Every precision loss was therefore routed
-- to major. A dropped article is a textbook precision loss.
--
-- v8 rewrites q3 in all three collection stages to ask about the RESULT only, never
-- about which category of content moved, and states the discriminator outright:
-- q1 asks what changed in the source, q3 asks whether the reader ends up believing
-- something false, and most q1 = true findings are q3 = false. The wrong-reading test
-- is sharpened from "write what the reader would misunderstand" to "write the
-- specific false thing the reader would take away" — "the reader may understand it
-- less precisely" is now explicitly not good enough to hold a q3.
--
-- This does not re-open the leniency this pipeline started with. The failures that
-- mattered — a mechanism reversed so the reader believes the RNA emits a signal
-- rather than being one, a marker for injury turned into a marker caused by
-- sunburn — pass the sharpened test easily: there is a concrete false belief to
-- write down. What no longer passes is a shade of precision.
--
-- Also added, one line, to the "not recorded" list: English articles and number have
-- no morphology in Chinese and normally go unrealised, so their absence is not a
-- deviation unless the reference actually changes. That is a fact about this language
-- pair rather than a detail about any one text, and it is the single most common
-- false positive between them.
--
-- v7 is deactivated, not deleted (AGENTS.md #9).
-- =====================================================================
...
NAATI 官方定义原文:
  ...
  q3 = 它 impacts on comprehension of the target text 吗?
       ...
       q1 与 q3 问的不是同一件事:q1 问【原文的什么变了】,q3 问【读者最后错没错】。
       【q1 = true 的偏差,绝大多数 q3 = false】——精度受损不等于理解出错,这正是官方 Minor 的定义。
...
每条同样要回答三问,只答 true/false,不要自己定等级:
  ...
  q2 = 它改变了这段文字的意图或功能吗?
  q3 = 它 impacts on comprehension 吗?
       只问【结果】,不问原文的什么变了:只读中文的读者,最后拿到的信息是不是错的。
       - true :读者会当真地相信一件与原文不符的事;或这句话读下来根本拼不出意思。
       - false:读者拿到的信息仍然正确,只是不够精确、不够自然、不够地道。
       q1 与 q3 问的不是同一件事:q1 问【原文的什么变了】,q3 问【读者最后错没错】。
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  q3 = impacts on comprehension of the target text 吗?
       只问【结果】,不问原文的什么变了:只读中文的读者,最后拿到的信息是不是错的。
       - true :读者会当真地相信一件与原文不符的事;或这句话读下来根本拼不出意思。
       - false:读者拿到的信息仍然正确,只是不够精确、不够自然、不够地道。
       q1 与 q3 问的不是同一件事:q1 问【原文的什么变了】,q3 问【读者最后错没错】。
...
-- Verify: exactly one ACTIVE grading row (version 8); v1-v7 kept but inactive.
