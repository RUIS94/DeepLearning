-- =====================================================================
-- Grading prompt v5 — stops the collection stages putting a dimension key into
-- errorCategory.
--
-- WHY (2026-09-04, third real run of the four-stage pipeline —
-- logs/ai-trace/00{11,12,13}_20260904-150*.md):
--
-- The proofread stage emitted `"errorCategory": "textual_norms", "dimensionKey":
-- "textual_norms"` — the dimension key duplicated into both fields. ValidateFindings
-- rejected it (correctly: error_category is a hard constraint checked against
-- error_taxonomies, AGENTS.md #1), the stage retried twice, produced the byte-identical
-- payload both times because temperature is 0, and the whole grading died. Four calls
-- and five minutes of real tokens lost to one mislabelled field.
--
-- The bait was in this file's own output contract. Evidence and sweep write the
-- dimension slot as an abstract reference ("<上方维度 key 之一>") and never failed;
-- proofread wrote it as a literal list, `"dimensionKey": "<textual_norms|language_
-- proficiency>"`, sitting one field to the right of an errorCategory placeholder that
-- was only an abstract description. The only concrete string in view got copied into
-- the nearest slot. v5 removes that literal, points each placeholder at the exact
-- section its values come from, and states under all three contracts that the two
-- fields draw on disjoint lists.
--
-- The prompt change is the prevention. The cure is in the handler: RunStageAsync now
-- appends the rejection reason to the re-prompt, so a content failure gets a retry
-- that can actually differ from the attempt that failed. Re-sending an identical
-- prompt at temperature 0 made the retry budget decorative — exactly the case
-- AGENTS.md #2 separates content failures from transport failures to handle.
--
-- Stages, model and template shape are otherwise unchanged from v4 (see
-- rebuild_grading_prompt_v4_comprehension_test.sql for the comprehension-test
-- recalibration, and rebuild_grading_prompt_v3_four_stage.sql for the four-stage
-- rationale).
--
-- v4 is deactivated, not deleted (AGENTS.md #9).
-- =====================================================================
...
-- Verify: exactly one ACTIVE grading row (version 5); v1-v4 kept but inactive.
