-- =====================================================================
-- Grading prompt v6 — stop the checklist memorising one article, and give the
-- "taken together" judgement numbers instead of a hunch.
--
-- WHY (2026-09-04, review before freezing a production prompt):
--
--  1. THE SWEEP CHECKLIST HAD MEMORISED THE TEST ARTICLE. Four of its items quoted
--     the sunburn text verbatim — "X is released as a signal", "a marker for
--     injury", radiation-is-cause/sunburn-is-effect, "the skin condition psoriasis".
--     Re-running that article would have scored well and proved nothing: those are
--     the answers, not a method, and not one of them fires on the next text. Worse,
--     it destroyed the only measuring instrument available — results on that article
--     could no longer be read as evidence that anything had improved. Each item is
--     now the abstract class plus a schematic example carrying no content words from
--     any real submission.
--
--  2. "TAKEN TOGETHER" WAS STILL PURE FEELING. Step 2b asked the model to sense how
--     long the text was and how much of it was affected. That is the judgement that
--     failed in the very first run — five errors in a 250-word text were waved off as
--     零星瑕疵. Models have no stable intuition for "a lot"; they have a perfectly
--     good one for "8 of 12 sentences". The handler now computes sentence count, word
--     count, how many sentences the evidence touches, and the per-dimension severity
--     histogram, and hands them over as fact (GradeSubmissionCommandHandler.
--     BuildCoverageNote). Cheapest available improvement to the softest step.
--
--  3. THE MODEL NO LONGER SPLITS THE SOURCE. It used to be told to "split on full
--     stops and number from 1", then report coverage against its own split — which
--     made the check decorative, because a model that stopped early could just emit
--     fewer rows. This very text opens "Why people get sunburn Sunburn shows that…",
--     a headline run into the body with no full stop, so the model's split started
--     out wrong anyway. SplitSentences now numbers them in code, the prompt shows the
--     numbered list, the model only fills in a status, and the handler rejects a
--     payload whose row count does not match.
--
--  4. THE PROOFREAD BAN WAS CUT TOO WIDE. Forbidding "指代不明确" alongside
--     completeness claims threw away the one class of defect that pass exists to
--     catch: an ambiguous modifier is judgeable from the Chinese alone, needs no
--     source, and is precisely the thread that leads to the mangled-mechanism
--     sentences the earlier runs missed. Only completeness claims are banned now.
--
-- Also changed outside this file, for the same review:
--   * DeriveSeverity: critical required q2 AND q3 AND scope, and q2 is effectively
--     always false for expository prose ("report this study" survives almost any
--     sentence-level slip), so the fourth severity was unreachable dead code. It is
--     now officially-Major (q2 or q3) whose reach passes its own sentence.
--   * MergeCollectedFindings: duplicates are matched by overlapping character ranges
--     in the translation rather than by snippet equality — three stages quote the
--     same defect at three different lengths, and a missed merge now inflates the
--     coverage figures point 2 relies on. The q3 wrong-reading is carried across the
--     merge too; without it a merged finding could assert a comprehension failure
--     with nothing behind it, and then be demoted for saying so.
--   * StandardOverride injection is capped and deterministically ordered — those
--     patches come from disputes, disputes are almost always about being marked too
--     harshly, so the set drifts one way over time and it feeds the collection stage.
--
-- Stages and shape are otherwise unchanged from v5.
-- v5 is deactivated, not deleted (AGENTS.md #9).
-- =====================================================================
...
2.【强制逐句核对】下方"原文逐句"已经替你编好号了。你【不需要也不要】自己切句,只需为其中【每一句】在 sentences[] 里回一行:
   该句无偏差写 "ok",有偏差写 "deviation" 并在 findings[] 里给出对应条目。
   行数必须与给出的句子数完全一致、n 从 1 连续——少一行系统就退回重做。这是为了防止"找到几条就停下"。
...
只输出以下 JSON,不要输出任何其他文字,也不要加代码块围栏:
{
  ...
  "findings": [
    {"id": "E1", "positionRef": "<定位,如 第二段第2句>", "sourceTextSnippet": "<原文片段,照抄>", "userTextSnippet": "<译文片段,照抄>", "errorCategory": "<上方错误类别清单里的 category_key 之一,不是维度名>", "dimensionKey": "<上方维度清单里的 key 之一>", "q1": <true|false>, "q2": <true|false>, "q3": <true|false>, "q3WrongReading": "<q3 为 true 时必填:一句话写出读者会误以为是什么;q3 为 false 时填 null>", "scopeBeyondSentence": <true|false>, "summary": "<≤20字中文定性>", "explanation": "<指出原文哪个成分被改变/遗漏/添加,以及三问的事实依据>", "suggestion": "<改法建议>"}
  ]
}
...
{
  "findings": [
    {"id": "S1", "positionRef": "<定位>", "sourceTextSnippet": "<原文片段,照抄>", "userTextSnippet": "<译文片段,照抄>", "errorCategory": "<上方错误类别清单里的 category_key 之一,不是维度名>", "dimensionKey": "<上方维度清单里的 key 之一>", "q1": <true|false>, "q2": <true|false>, "q3": <true|false>, "q3WrongReading": "<q3 为 true 时必填:一句话写出读者会误以为是什么;q3 为 false 时填 null>", "scopeBeyondSentence": <true|false>, "summary": "<≤20字中文定性>", "explanation": "<原文依据 + 三问的事实依据>", "suggestion": "<改法建议>"}
  ]
}
...
2. 先做两项【事实判定】(此时只判事实,不选 Band):
   ...
   b. 把该维度全部 minor + moderate 条目【合起来】看,它们 taken together 是否已构成该维度官方描述所说的 "significant impact on the overall precision / overall quality"?
...
-- Verify: exactly one ACTIVE grading row (version 6); v1-v5 kept but inactive.
