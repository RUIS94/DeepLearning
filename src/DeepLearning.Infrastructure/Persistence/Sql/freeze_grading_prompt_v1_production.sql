-- =====================================================================
-- The grading prompt, frozen as production v1.
--
-- This file is now the single source of truth for how a translation is graded. Every
-- rebuild_grading_prompt_* and fix_grading_* script before it is history: they remain
-- in the manifest so a fresh database still replays the same sequence, but this script
-- runs last and collapses whatever they produced down to one row numbered 1. A fresh
-- install and the live database therefore land on identical state — which is the whole
-- reason the version numbering restarts here rather than continuing at 11.
--
...
--   proofread — the translation ALONE, no source. Reading with the source alongside
--               hides Chinese-side faults, because you already know what it meant to say.
...
-- The three collection stages never see each other's findings; the handler unions them
-- and keeps the harsher reading of any duplicate, because a pass shown a list turns
-- into a validator that subtracts instead of a searcher that adds.
--
-- WHAT IT COST TO GET HERE, so none of it is re-learned by accident:
--   * A single call could not do this. One model asked to find, classify, rate and band
--     at once gave different answers on identical input, and its concrete detection
--     rules reliably out-competed the abstract Band text — it found errors first and
--     back-filled a Band to match.
--   * Severity is derived, never named by a model. Asked to answer the official
--     questions AND pick a level, it would answer "no, no" and write "major".
--   * There are two levels, not four. moderate and critical were this project's
--     inventions; across 55 real findings they produced 40 moderate / 14 major /
--     1 critical / zero minor — a four-point scale used as a two-point one, with both
--     points off where the rubric puts them.
--   * q3 must ask about the RESULT. When it listed the same content categories as the
--     old q1, every precision loss read as a comprehension failure and a dropped
--     article came back as a major error.
--   * Severity guidance is not recording guidance. Saying "most findings are not
--     severe" once halved recall: the stage marked 10 of 13 sentences ok and reported
--     two findings. Hence the firewall line in all three collection stages.
--   * The checklist teaches a method, not one article's answers. It briefly quoted the
--     text it was tuned on, which would have scored well on that text, generalised to
--     nothing, and destroyed the only instrument for telling whether a change helped.
--   * "Taken together" judges against counted facts. The handler computes sentence
--     count, word count, how many sentences the evidence touches and the per-dimension
--     Major/Minor split, because models have no stable intuition for "a lot" but a fine
--     one for "8 of 12 sentences".
--
-- Last measured on this text before freezing: 24 raw findings across the three
-- collection stages, 19 after merge, 1 Major and 18 Minor.
--
-- To change the rubric from here, edit the live row via
-- PUT /api/v1/prompt-templates/{id} for a quick trial, then fold the change back into
-- THIS file and bump its version — otherwise the next fresh database silently gets the
-- old wording, which is exactly how the v1 incident started.
-- =====================================================================

BEGIN;

-- Not "deactivate": this is the freeze, and leaving nine superseded drafts behind is
-- how the wrong one gets picked up later. Their text is in git, which is where a
-- superseded prompt belongs.
