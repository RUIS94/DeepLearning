...
| word_count | INT | 原文（`source_text`）词数，见下方实现补记 |
...
实现补记（2026-09-02）：手工导入题目时 `word_count` **由后端自动计算，不再接受调用方传入**——`ImportUserQuestionCommandHandler` 用 `WordCountCalculator.Count(source_text)` 覆写。计数规则：每个中日韩表意字符/假名记 1 词（无空格可分），其余文种按「字母或数字的连续段」记 1 词（`state-of-the-art` = 4，标点与空白只作分隔）。只数 `source_text`（待译原文），不含 `title`、`brief`，TaskB 的 `flawed_translation_text` 也不计入。对当前 NAATI CT 英译中（英文原文）精确，将来加中译英方向仍合理。`ImportUserQuestionCommand` / `ImportUserQuestionRequest` 已移除 `WordCount` 字段。AI 出题路径（`GenerateQuestionCommandHandler`）不变，仍取 LLM 返回的 `wordCount`。
...
> 实现补记（2026-09-01）：管理后台对本表**唯一开放的写操作**是 `POST /api/v1/standard-overrides/{id}/deprecate`（把某条 `status` 置为 `deprecated`）。不提供编辑、不提供物理删除——「只增不改、审计链」原则不变（`deprecated` 本就是合法状态，`previous_override_id` 链不动）。相较之下，`question_bank_categories` 与 `prompt_templates` 两张配置表在同一轮加了完整的 `PUT`/`DELETE`（管理 UI 需要），是对「配置表只增不改」惯例的有意放宽。
